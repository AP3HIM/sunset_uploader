...
def main():
    ...
    # New: --mode controls what PAG does
    # "full"             — legacy full PAG flow (opens Chrome, does everything)
    # "file-select-only" — new hybrid: just select the file, DOM handles the rest
    parser.add_argument('--mode', type=str, default="full",
                        choices=["full", "file-select-only"])
    ...
    if mode == "file-select-only":
        ...
        if platform_arg == "tiktok":
            ...
            print(f"Unknown platform for file-select-only: {platform_arg}")

        print("File selection complete. Exiting PAG.")
        return

    # ── Legacy full mode: opens Chrome and does everything ──────────────────────
    print(f"Launching full upload for: {platforms}")
    launch_chrome()
    time.sleep(1)
